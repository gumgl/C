...
void main(void)
{
	...
	while (true)
	{
		selected = -1; // Select the first card
		...
		while (true) // Choose which cards to hold
		{
			...
			hand.PrintHand(selected); // Draw cards in hand side-by-side with color
			...
			while (true)
			{
				...
				if (input == KEY_LEFT)
					selected = (selected <= 0) ? HAND_SIZE - 1 : selected - 1;
				else if (input == KEY_RIGHT)
					selected = (selected >= HAND_SIZE - 1) ? 0 : selected + 1;
				if (input >= 49 && input <= 48 + HAND_SIZE)
				{
					selected = input - 49;
					input = KEY_ENTER;
				}
				if (input == KEY_LEFT || input == KEY_RIGHT || input == KEY_ENTER || input == KEY_SPACE)
					break;
				else if (input == KEY_ESCAPE && cardsHeld < MIN_HOLD)
					cout << endl << lang(currLang, 17) << " " << MIN_HOLD << " " << lang(currLang, 18) << ((MIN_HOLD > 1) ? "s" : "");
