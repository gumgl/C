...
{
	/*theDeck[0].Set(Six, Hearts);
	theDeck[1].Set(Six, Spades);
	theDeck[2].Set(Ten, Hearts);
	theDeck[3].Set(Ten, Hearts);
	theDeck[4].Set(Ten, Hearts);

	Print();*/

	const int MIN_PAIR = Jack;
	int count;
	...
	for (count = 0; count < 13; count ++) // Look for a straight, pairs, 3-of-a-kind and 4-of-a-kind
	{
		if (faces[count] == 1)
			following ++;
		else if (following >= 1)
	...
	if (dups[2] == 2)
		return TwoPair;
	if (dups[2] == 1 && pairIndex >= MIN_PAIR)
		return Pair;
